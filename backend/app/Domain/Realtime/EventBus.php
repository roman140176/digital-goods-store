...
{
    /**
     * Единый канал pg_notify для всех топиков. Стример (StreamServer) читает
     * topic из самой строки журнала и сам раздаёт событие только тем
     * подключениям, что на этот топик подписаны.
     *
     * Константа публичная, потому что у канала ровно две стороны — эта
     * публикация и LISTEN в стримере, — и имя обязано быть у них общим.
     * Продублированный в двух файлах литерал давал бы худший из возможных
     * отказов: переименовали в одном месте, стример молча оглох, а
     * догоняющий опрос раз в две секунды это замаскировал бы — канал
     * «работает», просто на две секунды медленнее.
     */
    public const CHANNEL = 'storefront';
