...
final class OrderPresenter
{
    ...
    private static function reservation(?StockUnit $unit): ?array
    {
        ...
        return [
            ...
            // ceil() плавающей разницы Carbon вместо вычитания целых меток
            // getTimestamp() — не смена поведения: reserved_until всегда
            // целые секунды (timestamp(0)), а для целого дедлайна D тождество
            // D - floor(x) = ceil(D - x) верно при любом x, то есть старое
            // выражение и это дают одно и то же число. ceil() просто
            // выражает намерение «никогда не занижать остаток» явно, а не
            // как побочный эффект целочисленного вычитания: изменись когда-
            // нибудь точность колонки и дедлайн перестань быть целым числом
            // секунд — ceil() продолжит держать инвариант, а старое
            // выражение молча перестанет.
