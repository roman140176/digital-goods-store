...
// Сценарий настраивает сбой именно у поставщика A и рассчитывает, что заказ
// на KEY-CS2-PRIME пойдёт именно через него: самое дешёвое предложение
// позиции сидер закрепляет за supplier_id='a' детерминированно (см. задачу
// 2 второго этапа). Если это предложение уже раскуплено более ранним
// сценарием (у него намеренно скудный сток — см. race-double-click.php),
// bestOfferFor() молча уедет на следующее по цене предложение — оно вполне
// может оказаться за поставщиком B, и вся проверка окажется не про то.
